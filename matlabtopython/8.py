...
class PIDController:
    ...
    def update(self, setpoint, process_variable, time_step):
        ...
        # 使用滤波器对误差进行平滑处理
        self.filtered_error = (self.filter_coefficient * error +
                               (1 - self.filter_coefficient) * self.filtered_error)
        # 更新积分项
        self.integral += self.filtered_error * time_step
        # 计算微分项
        derivative = (self.filtered_error - self.prev_error) / time_step
        # 存储当前滤波后的误差为上一个时间步的误差
        self.prev_error = self.filtered_error
        # 根据PID公式计算输出
        output = self.kp1 * self.filtered_error + self.ki1 * self.integral + self.kd1 * derivative
        return output
...
def fcn(ne, d0, ty1, tj, p1, t1):
    ...
    rho1 = 1210.86211811911 - 1.59793418318532 * t1 - 35.8884167229680 * p1 +0.313417735621854 * p1 * t1 -0.000455489263743170 * t1 ** 2 -0.867613946591206 * p1 ** 2 +0.00280501652204646 * p1 ** 2 * t1 -0.000615644347189089 * p1 * t1 ** 2

    drho1 = -35.8884167229680 + 0.313417735621854 * t1 -2 * 0.867613946591206 * p1 + 2 * 0.00280501652204646 * p1 *t1 ** 2 - 0.000615644347189089 * t1 ** 2
    # 计算 H0,  H1, dHT1, dHP1
    ...
    d1 = cmath.sqrt(rho1 * (p1 - pout) / ksi1).real
    ...
    return d1, dp1, d_t1, dty1, dtj, p0, pout, h0, h1, ksi1, ty1
...
def fcn3(pd1, pt):
    #print("pd1:", pd1, "pt:", pt)
    plus = 0.08764256902732083 * pd1 ** 2 - 1.8391411470804848 * pd1 + 18.439682689872793
    c0 = 38 / 2.8
    ds = c0 * plus * (abs(pd1 - pt)) ** 0.5 + 200
    if ds > 600:
        ds = 600
...
def main():
    ...
    main_steam_flow = main_steam_flows[0]
    ne = nes[0]  # 负荷值初始化
    d0 = 0  # 给水流量的初始值
    process_variable2 = 0  # 副控制器的变量初始值
    ...
    v = 50
    pd = 18000000
    initial_condition = 0  # dld_dt的积分初始值
    pd = pd * 0.000001
    ...
    while current_time <= total_time:
        ...
        d0 = output2
        # 调用龙格-库塔函数进行积分
        d1, dp1, d_t1, dty1, dtj, _, _, _, _, _, _ = fcn(ne, d0, ty1, tj, p1, t1)
        p1, _ = runge_kutta_4(lambda p1, t: dp1, t_0, p1, h, n_steps)
        p1 = p1[-1]  # 更新p1的初始条件为积分后的最后一个值
        t1, _ = runge_kutta_4(lambda t1, t: d_t1, t_0, t1, h, n_steps, 1, 1000)
        t1 = t1[-1]  # 更新t1的初始条件为积分后的最后一个值
        ty1, _ = runge_kutta_4(lambda ty1, t: dty1, t_0, ty1, h, n_steps, 1, 2000)
        ty1 = ty1[-1]  # 更新ty1的初始条件为积分后的最后一个值
        tj, _ = runge_kutta_4(lambda tj, t: dtj, t_0, tj, h, n_steps, 10, 1000)
        tj = tj[-1]  # 更新tj的初始条件为积分后的最后一个值
        d1 = fcn(ne, d0, ty1, tj, p1, t1)
        dsm = d1[0]
        dgr = ds
        deta_pd, deta_v, dld_dt, t_s, deta_vs = fcn2(dsm, pd, v, dxj, dgr, deta_xjs)
        # print("pd:", pd)
        #print("deta_pd:", deta_pd)
        pd_integral, _ = runge_kutta_4(lambda pd, t: deta_pd, t_0, pd, h, n_steps, 6000000, 30000000)
        pd = pd_integral[-1]
        gain1 = 0.000001
        pd = pd * gain1  # 应用增益得到Pd的值
        v, _ = runge_kutta_4(lambda v, t: deta_v, t_0, v, h, n_steps, 0, 225)
        v = v[-1]
        ...
        pd1 = pd
        ds = fcn3(pd1, pt)
        current_loop_time = time.time()  # 获取当前循环中的时间
        if current_loop_time - last_record_time >= time_interval:
            output_signals.append(output_signal)  # 记录输出信号
            time_steps.append(current_time)  # 记录当前模拟时间
            last_record_time = current_loop_time  # 更新最后记录时间
            i += 1

        # 更新当前时间
        current_time += current_loop_time - start_time
