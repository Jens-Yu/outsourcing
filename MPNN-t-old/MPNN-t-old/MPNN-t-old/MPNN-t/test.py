...
# Your other imports and utility functions here...
...
class NodePropLayer(nn.Module):  # 该类为MPNN---------重点看！！！
    ...
    def __init__(self, args, name='propLayer'):
        ...
        if self._layer_norm:
            ...
            self.se_layer = SELayer(self.node_emb_dim)
    ...
    def build_model(self):
        ...
        layer = []
        layer.append(
            nn.Linear(self.node_emb_dim * 2 + self.edge_type_emb_dim, self.msg_emb_dim))  # 原始消息嵌入由两个节点嵌入和边类型嵌入拼接而成
    ...
    def _compute_node_update(self, node_features, update_msg):
        ...
        if self._node_update_type == 'gru':
            ...
            # 应用SE层
            se_layer = SELayer(mlp_output.size(1))  # mlp_output.size(1) 返回特征维度
            mlp_output = se_layer(mlp_output.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1)
...
class MLM(nn.Module):  # 模型整体构造,主要包括上述两类  图编码和MPNN
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self._block_prop_layers):
            ...
            block_node_features1 = layer(block_node_features1, block_edge_features1, block_node_from_idx,
                                         block_node_to_idx)
        ...
        node_graph_emb = torch.div(
            unsorted_segment_sum(self.block_node_prop_features, node_graph_idx,
                                 node_graph_idx[node_graph_idx.shape[0] - 1].item() + 1),
