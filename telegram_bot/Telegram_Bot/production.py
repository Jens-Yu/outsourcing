...
from mongodb import productions_collection, users_collection
...
async def input_production_quota(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        # 创建负责人选择按钮
        keyboard = [[InlineKeyboardButton(member.get('username', 'No Username') or 'No Username', callback_data=str(member['_id']))]
...
async def principal_button_callback(update: Update, context: CallbackContext) -> int:
    ...
    if data == 'confirm_selection':
        ...
        await query.message.reply_text("请输入产品开始的日期 (格式：YYYY-MM-DD):")
        return PRODUCTION_START_DATE
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(text=f"当前选择的负责人有：{', '.join(selected_names)}。点击 '完成选择' 继续。", reply_markup=reply_markup)
...
async def save_production(update: Update, context: CallbackContext) -> int:
    production_data = {
        ...
        # 'principals': context.user_data['selected_principals'],
...
productions_conv_handler = ConversationHandler(
    ...
    states={
        ...
        PRODUCTION_GENRE: [CallbackQueryHandler(genre_button_callback, pattern='|'.join([genre.name for genre in Genre]))],
