...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                self.insert(data)
                rewards_all += rewards
                compressed_actions = actions.squeeze()
                lists[0].append(compressed_actions[0])
                lists[1].append(compressed_actions[1])
            ...
            print('rewards:', rewards_all // self.episode_length)
    ...
    def collect(self, step):
        ...
        for agent_id in range(self.num_agents):
            ...
            # print('self.buffer[0].share_obs:',len(self.buffer[0].share_obs))
            #print('action:', action)
            # [agents, envs, dim]
            values.append(_t2n(value))
            action = _t2n(action)
            #print('action after _t2n:', action)
            # rearrange action
        ...
        #MultiDiscreteprint('actions:', actions, '\n', 'actions_env:', actions_env)
        return (
            values,
            actions,
            action_log_probs,
